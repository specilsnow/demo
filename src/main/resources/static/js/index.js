...
//声明一个来保存位置ID
var lableid = 0;
var temperatureListAfter="",temperatureListBefore,resistanceListBefore,resistanceListAfter;
...
function reservationData(visitNo) {
    ...
    $.post(url, postdata, function (data) {
        ...
        $("#patientAge").val(data.patient.ye + "岁");
        ...
        $("#imgList .imglist").remove()
        for (var ind = 0; ind < data.emrImgifos.length; ind++) {
            $("#imgList").append("<div class='imglist'> <div> <img src='" + data.emrImgifos[ind].imgUrl + "'/></div><span>" + data.emrImgifos[ind].imgName + "</span> </div>")
        };
        if(data.treatData!=undefined){
            temperatureListBefore=data.treatData.temperatureListBefore;
            temperatureListAfter=data.treatData.temperatureListAfter;
            resistanceListBefore=data.treatData.resistanceListBefore;
            resistanceListAfter=data.treatData.resistanceListAfter;
            $("#xwmhData").show();
            $("#rightToolsTab li").removeClass("active").eq(0).addClass("active");
        }else {
            $("#xwmhData").hide()
            console.log("数据不存在")
        }
...
function showitem(id, name, obj) {
    ...
    selectedRecipelName.push(name);
    $("#recipelName").before("<span id='" + id + "'>" + name + "<i class='fa fa-remove'  onclick='removerecipel(\"" + id + "\",this)'></i></span>");
    $("#recipelName").val("");
    $("#emrform").attr("action", contextPath + "/iteminfo");
    $("#recipelbyid").val(id);
    $("#recipelNames").val(selectedRecipelName);
    $("#emrform").ajaxSubmit(function (e) {
        $("#materialdiv").html(e);
        $("#selectedrecipel").append("<label class='col-md-12' id='" + id + "' style='padding:0 5px' onclick='removerecipel(\"" + id + "\",this)'>" + name + "</label>");
        // if($(window).width()<=768){
        $(".selectedrecipel").append("<label class='col-md-12' id='" + id + "' style='padding:0 5px' onclick='removerecipel(\"" + id + "\",this)'>" + name + "</label>");
        // }
        getmedical();
...
function removerecipel(id, obj) {
    ...
    $("#emrform").ajaxSubmit(function (e) {
        $("#materialdiv").html(e);
        // $(obj).remove();
        $(".selectedrecipel label").each(function (i, item) {
            // console.log(item)
            if ($(item).attr("id") == id) {
                $(item).remove();
            }
        })
        $(".recipelName>span").each(function (i, item) {
            // console.log(item)
            if ($(item).attr("id") == id) {
                $(item).remove();
            }
        ...
        for (var i = 0; i < selectedRecipel.length; i++) {
            if (id == selectedRecipel[i]) {
                ...
                selectedRecipelName.splice(i, 1);
                $("#recipelNames").val(selectedRecipelName);
            }
        }
        getmedical();
...
function searchNextPagematerial(a, event) {
    ...
    var materialsjson = JSON.parse(storage.getItem("materials"));
    // console.log(a + ":")
    var materialname = Enumerable.from(materialsjson)
        .where("x=>x.pinyin.toLowerCase().indexOf('" + q + "')!=-1 ||x.name.indexOf('" + q + "')!=-1 ")
...
function share() {
    if (iflag) {
        ...
        if (ybuser == null || ybuser == "" || ybuser == undefined) {
            ...
            if ($("#diseaseSS").val() != "" || $("#disease").val() != "") {
                $("#emrform").data('bootstrapValidator').enableFieldValidators("diseasea", false);
                $("#diseaseSS").parents(".form-group").addClass("has-success");
            }
            if ($(".recipelName span").length > 0) {
                $("#emrform").data('bootstrapValidator').enableFieldValidators("recipels[0].name", false);
                $("#recipelName").parents(".form-group").addClass("has-success");
            }
...
function SaveNoVoild() {
    ...
    if (ybuser == null || ybuser == "" || ybuser == undefined) {
        ...
        $("#chiefComplaint").val(diseaseSS);
        console.log($("#chiefComplaint").val())
        // return
...
$(".VTD .tongue").on("click", "div ul li", function () {
    ...
    if ($(this).parents(".shezhi-div").index() == 2) {
        ...
        for (var cyli = 0; cyli < $(".changyong ul li").length; cyli++) {
            if ($(".changyong ul li").eq(cyli).text() == thistext) {
                $(".changyong ul li").eq(cyli).toggleClass("check");
            }
        }
    }

    if ($(".sheshen ul li.check").length != 0) {
...
/* 发送给药房人员的微信上 */
function pushWxMsg2Yf(visitNo) {
    $.ajax({
        url: contextPath + "/pushWxMsg2Yf",
        data: {
            visitNo: visitNo
        },
        success: function (e) {
            layer.msg('已将该处方发送到药房人员微信！')
        },
        error: function (e) {
            alert(e)
        }
...
function chuan(num) {
    ...
    if (defaultage.substring(0, defaultage.length - 1) > 120) {
        ...
        $("input[name='age']").parents(".form-group").removeClass("has-error").addClass("has-success");
        $("input[name='age']").next("i").removeClass("glyphicon-remove").addClass("glyphicon-ok");
        ;
    }
    var evt = event || window.event;
    if (evt.stopPropagation) {
...
$(function () {
    MaxMe(document.getElementsByClassName('ff')[0]);
    MaxMe(document.getElementById("disease"));
    MaxMe(document.getElementById("diseaseSS"));
    $('.dropdown-toggle').dropdown()
    /*诊所列表*/
    // $.ajax({
    //     url:contextPath+"/getDoctorOrgList",
    //     success:function (e) {
    //         $(e).each(function (i,item) {
    //             $(".doctorname").append("<option value='"+item.clinicId+"'>"+item.name+"</option>")
    //         })
    //     }
    // })
})
...
function chuan1(num) {
    ...
    $("input[title='jishu']").parents(".form-group").removeClass("has-error").addClass("has-success");
    $("input[title='jishu']").next("i").removeClass("glyphicon-remove").addClass("glyphicon-ok");
...
function chuanyf(num) {
    var defaultjs = $(".time-content").eq(objparentIndex - 2).find("p span").eq(objIndex).find("input").val();
    if (num == "del") {
        defaultjs = defaultjs.substring(0, defaultjs.length - 1);
    } else {
        defaultjs += num;
    }
    $(".time-content").eq(objparentIndex - 2).find("p span").eq(objIndex).find("input").val(defaultjs);
    yfhx();
    zfhx();
    sjhx();
    aihx();
...
$("input[name=sex]").click(function () {
    ...
    if (inputId == "optionsRadios26") {
        $(".menstruation").css("display", "inline-block");
        $(".menstruation").attr("disabled", false)
    } else {
        $(".menstruation").hide();
        $(".menstruation").attr("disabled", true)
    }
})

/*选择跟诊医生*/
function doctorchoose(obj, id) {
...
function departmet() {
    ...
    var url = contextPath + "/therapyType"
...
/*病名下拉框*/
$("#symptomdiseases").autocomplete({
    serviceUrl: "/recommend/disease",
    paramName: "keyword",
    dataType: "json",
    ...
    onSelect: function (suggestion) {
        $("input[name=symptom]").parents(".form-group").addClass("has-success").removeClass("has-error").find("i.form-control-feedback").hide();
        autoPrompt($("#symptomdiseases"));
    },
    minChars: 0,
...
/*证型下拉框*/
$("#symptommould").autocomplete({
    serviceUrl: "/recommend/symptommould",
    paramName: "keyword",
    dataType: "json",
    transformResult: function (response) {
        return {
    ...
    onSelect: function (suggestion) {
        $("input[name=symptommould]").parents(".form-group").addClass("has-success").removeClass("has-error").find("i.form-control-feedback").hide();
        autoPrompt($("input[name=symptommould]"));
    },
    minChars: 0,
    onSearchError: function (query, jqXHR, textStatus, errorThrown) {
        if (jqXHR.responseText.trim() == "timeout") {
...
$("#therapy").autocomplete({
    ...
    onSelect: function (suggestion) {
        $("#therapy").parents(".form-group").addClass("has-success").removeClass("has-error").find("i.form-control-feedback").hide();
    },
    minChars: 0,
    onSearchError: function (query, jqXHR, textStatus, errorThrown) {
        if (jqXHR.responseText.trim() == "timeout") {
...
/*处方名下拉框*/
function addRecipelName(obj) {
    var value = $(obj).val();
    console.log(value)
    var url = contextPath + "/recommend/recipelName?keyword=" + value;
    $.get(url, function (data) {
        $(".recipelList").show();
        $(".recipelList ul li").remove()
        $(data).each(function (i, item) {
            // console.log(item)
            if (item.notice == "1") {
                $(".recipelList ul").append("<li onclick='showitem(\"" + item.id + "\",\"" + item.name + "\",this)'>" + item.name + "<span>医生用户</span></li>")
            } else {
                $(".recipelList ul").append("<li onclick='showitem(\"" + item.id + "\",\"" + item.name + "\",this)'>" + item.name + "<span>通用</span></li>")
...
function showBigImg(obj) {
    layer.open({
        ...
        shadeClose: false,
        area: ['90%', '90%'],
        content: '<img src="' + $(obj).attr("src") + '" style="max-width: 100%"/>' //iframe的url
    });
}

/*切换诊所*/
$("select.doctorname").change(function () {
    console.log($(this).val() + "--" + $(this).find("option:selected").text());
    var clinicId = $(this).val();
    var clinicName = $(this).find("option:selected").text();
    var clinicUrl = contextPath + "/switchClinic?clinicId=" + clinicId + "&clinicName=" + clinicName
...
function additemName(obj) {
    ...
    if($(obj).attr("type")=="recipel"){
        showitem($(obj).attr("id"),$(obj).text(),$(obj));
    }else{
        $(obj).parent().siblings(".form-control:first").val($(obj).text());
        autoPrompt($(obj).parent().siblings(".form-control:first"));
        var objname=$(obj).parent().siblings(".form-control:first").attr("name")
        $("#emrform").data('bootstrapValidator').enableFieldValidators(objname, false);
...
/*症状修改提示其它*/
function autoPrompt(obj) {
    var pattern = new RegExp("[\u4E00-\u9FA5]+");
    var itemVal = $(obj).val();
    var itemType = $(obj).attr("data-type");
    if(pattern.test(itemVal)){//判断是否是中文，如果是掉接口
        $.ajax({
            url: contextPath + "/recommendate/disease",
            data: {
                type: itemType,
                keyword: itemVal
            },
            success: function (e) {
                ...
                if (e.hasOwnProperty("data")) {
                    autoPromptDisease(e.data.diseases);
                    autoPromptSymptom(e.data.symptoms);
                    autoPromptSymptommould(e.data.symptommoulds);
                    autoPromptRecipelName(e.data.recipels)
                }
            },
            error: function (e) {
                alert(e)
...
/*症状*/
function autoPromptDisease(data) {
    $("#diseaseSS").siblings("p").find("span").remove()
    $(data).each(function (i, item) {
        $("#diseaseSS").siblings("p").append('<span onclick="additemName(this)">' + item.diseasename + '</span>')
    })
}

/*病名*/
function autoPromptSymptom(data) {
    $("#symptomdiseases").siblings("p").find("span").remove()
    $(data).each(function (i, item) {
        $("#symptomdiseases").siblings("p").append('<span onclick="additemName(this)">' + item.symptomname + '</span>')
    })
}

/*证型*/
function autoPromptSymptommould(data) {
    $("#symptommould").siblings("p").find("span").remove()
    $(data).each(function (i, item) {
        $("#symptommould").siblings("p").append('<span onclick="additemName(this)">' + item.symptommouldname + '</span>')
    })
}

/*处方*/
function autoPromptRecipelName(data) {
    $("#recipelName").siblings("p").find("span").remove()
    $(data).each(function (i, item) {
        $("#recipelName").siblings("p").append('<span onclick="additemName(this)" id="'+item.recipelid+'" type="recipel">' + item.recipelname + '</span>')
    })
}
//穴位敏化图
var option = {
...
$(".portlet.light>.portlet-title>.pull-right").click(function () {
    console.log(temperatureListAfter=="")
    $(this).find("span").toggleClass("fa-angle-up").toggleClass("fa-angle-down");

    if(temperatureListAfter!=""){
        $(this).parent().next().slideToggle("500");
        setTimeout(function () {
            var temperature = echarts.init(document.getElementById('temperature'));
            var resistance = echarts.init(document.getElementById('resistance'));
            resistance.setOption(option);
            resistance.setOption({
                title: {
                    text: '电阻对比图',
                    textStyle:{
                        fontWeight:"normal",
                ...
                series: [
                    {
                        data:resistanceListBefore
                    },
                    {
                        data:resistanceListAfter
                    }
                ]
            });
            ...
            temperature.setOption({
                ...
                series: [
                    {
                        data:temperatureListBefore
                    },
                    {
                        data:temperatureListAfter
                    }
                ]
            });
        },10)
