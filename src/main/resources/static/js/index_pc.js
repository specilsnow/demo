...
function findEmrHistoryByName(name, date, type) {

    $("#histroy-list").load(contextPath + "/emr/findlistpageemr?patientName=" + name + "&datestring=" + date, function (response, status, xhr) {
        if (status == "success") {
            if (response == "") {
            } else {
...
function historypeople() {
    console.log("加载病人处方页")
    $("#histroy-list").load(contextPath + "/emr/findlistpageemr?patientName=" + $('#tab2name').val() + "&datestring=" + $('#tab2date').val());
...
$(".VTD .vein .tab-pane table td div a").click(function () {
    ...
    if (trindex == 0 || trindex == 2) {
        ...
        $(this).parent().siblings().removeClass("xuan");
...
$(".VTD .vein .tab-pane table td div p span").click(function () {
    ...
    if ($(this).hasClass("ch")) {
        ...
        if (trindex == 0 || trindex == 2) {
            ...
            $(this).parent().parent().siblings().removeClass("xuan");
            $(this).parent().parent().siblings().find("p span").removeClass("ch");
        }
...
function chuanyf(num) {
    ...
    var defaultjs = $(".time-content").eq(objparentIndex - 2).find("p").find("span").eq(objIndex).find("input").val();
    if (num == "del") {
        defaultjs = defaultjs.substring(0, defaultjs.length - 1);
    } else {
        defaultjs += num;
    }
    $(".time-content").eq(objparentIndex - 2).find("p").find("span").eq(objIndex).find("input").val(defaultjs);
    yfhx();
    zfhx();
    sjhx();
    aihx();
